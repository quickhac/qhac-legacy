        ...
        api: {
            ...
            disambiguate: {
                ...
                isRequired: function (dom) {
                    var buttons = dom.find('.sg-button').toArray();
                ...
                getDisambiguationChoices: (function (dom) {
                    return dom.find('.sg-student-picker-row').map(function (i) {
                        return ({
                            name: i.find('.sg-picker-student-name')[0].innerText,
                            id: i.find('input[name=studentId]')[0].value
                        });
                    });
                })
            },
        ...
        function getDisambigChoices(doc) {
            ...
            if (district.api.disambiguate.isRequired($dom)) {
                ...
                if (district.api.disambiguate.pickerLoadsFromAjax) {
                    new XHR('GET', district.api.disambiguate.url).success(function (doc) {
                        var $dom = document.createElement('div');
                        $dom.innerHTML = doc;
